...
describe("Chinese translation", () => {
  ...
  it("includes all the validation results that chinese does", () => {
    const instance = { extend: jest.fn() };
    locales.en(instance);
    locales[locale](instance);
    const chineseMessages = Object.keys(
      instance.extend.mock.calls[0][0].locales.en
    );
    const localizedMessages = Object.keys(
      instance.extend.mock.calls[1][0].locales[locale]
    );
    expect(chineseMessages).toEqual(localizedMessages);
  });
});
